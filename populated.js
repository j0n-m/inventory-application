...
async function createProductCategories() {
  ...
  await Promise.all([
    ...
    categoryCreate(2, "House Appliance"),
  ]);
}
async function categoryCreate(arrIndex, name, description = '') {
  const category = new ProductCategory({
    name,
    description
  });
  await category.save()
  productCategories[arrIndex] = category;
}

async function createProducts() {
  console.log('Creating sample products');
  await Promise.all([
    productCreate(0, 'TV', 250.00, 4, '32in HD Smart TV', productCategories[0]),
    productCreate(1, 'Blue Jeans', 59.99, 12, '32x32 Men blue jeans', productCategories[1]),
    productCreate(2, 'Toaster', 39.99, 8, 'Four slice toaster', productCategories[2]),

  ])
}
async function productCreate(arrIndex, name, price, quantity, description, category) {
